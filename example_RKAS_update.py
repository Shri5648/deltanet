...
from deltanet_RKAS_update import DeltaBlock
...
def generate(model, prompt, seq_len=10):
    model.eval() 
    S = None
    r = None # RKAS requires tracking the residual
    last_output = None
    
    with torch.no_grad():
        # Process the prompt to build the hidden state S and residual r
        for c in prompt:
            print(c, end="")
            x = char_to_tensor(c)
            # Update signature to handle the 3-item tuple (output, state, residual)
            last_output, S, r = model.step(x, S, r)
        ...
        for i in range(seq_len):
            ...
            # Maintain the residual r through the generation loop
            x, S, r = model.step(x_input, S, r)
